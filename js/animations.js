(function(){
  ...
  function initProjectParallax(){
    ...
    sections.forEach(section=>{
      if(section.dataset.parallaxReady)return;
      section.dataset.parallaxReady='1';
      ...
      function update(){
        ...
        const range=viewport + r.height;
        const progress=clamp((viewport-r.top)/range,0,1);
        layers.forEach(layer=>{
          const speed=parseFloat(layer.dataset.parallaxSpeed)||.3;
          const offset=(progress-.5)*speed*180;
      ...
      layers.forEach(layer=>layer.addEventListener('load',request,{once:true}));
      update();
    });
  }
