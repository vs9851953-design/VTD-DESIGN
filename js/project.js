...
function projectMedia(p){
  const raw=uniq([...(Array.isArray(p.gallery)?p.gallery:[]), ...(Array.isArray(p.videos)?p.videos:[]), ...(p.video?[p.video]:[])]);
  const videos=raw.filter(isVideo);
  const images=uniq((Array.isArray(p.gallery)?p.gallery:[]).filter(src=>!isVideo(src)));
  if(!images.length){
    const fallback=uniq([p.cover,p.thumb]).filter(src=>!isVideo(src));
...
function relatedMedia(p){
  ...
  return p.thumb||p.cover||m.images[0]||'assets/placeholder.jpg';
}
...
function render(p,projects){
 ...
 const conceptImage=p.concept||p.conceptImage||null;
 root.innerHTML=`
...
<section class="concept"><div class="concept-inner ${conceptImage?'':'no-concept-media'}">${conceptImage?`<div class="concept-media">${img(conceptImage,p.titulo)}</div>`:''}<div class="concept-copy"><h2>Conceito</h2><p>${safe(p.conceito||p.conceptText||p.descricao||'')}</p>${p.feedback?`<div class="feedback">“${safe(p.feedback)}”<br><strong>${safe(p.feedbackAutor||'')}</strong>${p.feedbackCargo?` · ${safe(p.feedbackCargo)}`:''}</div>`:''}</div></div></section>
