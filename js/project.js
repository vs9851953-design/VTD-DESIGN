...
function video(src,title,cls='',poster=''){return `<video class="${cls}" controls muted loop playsinline preload="auto" ${poster?`poster="${safe(poster)}"`:''} aria-label="${safe(title)}"><source src="${safe(src)}" type="${String(src).toLowerCase().endsWith('.webm')?'video/webm':'video/mp4'}">Seu navegador não suporta vídeo.</video>`}
...
function setupProjectVideos(){
 document.querySelectorAll('.video-main video').forEach(v=>{
   const play=()=>{v.muted=true;const promise=v.play();if(promise)promise.catch(()=>{});};
   v.addEventListener('loadedmetadata',play,{once:true});
   v.addEventListener('canplay',play,{once:true});
   v.addEventListener('click',()=>{if(v.paused)play();});
 });
}
